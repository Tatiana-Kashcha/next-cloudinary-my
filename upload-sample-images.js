...
const assets = require("./scripts/assets.json");
const images = require("./scripts/images.json");
const videos = require("./scripts/videos.json");
...
async function uploadAssets(assets, { folder, resourceType }) {
  ...
  console.log(
    `Uploading ${assets.length} assets to cloud "${process.env.CLOUDINARY_CLOUD_NAME}" in folder ${folder}...`
  );
